...
class SuscripcionesModelo
{
    public static function mdlGuardarSuscripciones($scs)
    {
        try {
            //code...
            $sql = "INSERT INTO tbl_suscripciones_scs (scs_correo, scs_clave, scs_telefono, scs_nombre, tenantid) VALUES(?,?,?,?,?)";
            $con = Conexion::conectar();
            $pps = $con->prepare($sql);
            $pps->bindValue(1, $scs['scs_correo']);
            $pps->bindValue(2, $scs['scs_clave']);
            $pps->bindValue(3, $scs['scs_telefono']);
